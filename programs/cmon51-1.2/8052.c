...
void step_and_break (void) __interrupt(3);
...
void last_interrupt (void) __interrupt(5) __naked {}  //timer 2

extern volatile __xdata unsigned int  PC_save;
...
void asm_code_sfr (void) __naked
{
    __asm__(
    	"_code_sfr:"
		"; Write  sfr"
...
void main (void)
{
	__asm__("ljmp _do_cmd _endasm; //All the work is done in cmon51.c");
}
...
void loadintelhex (void)
{
	...
	unsigned char * xptr;
	unsigned char __code * cptr=0;
	__bit errorbit, errorprog;
	...
	while(1)
	{
		...
		if(n==':')
		{
			...
			xptr=(unsigned char __xdata *)address;
			//Use cptr to verify the byte is visible as code memory
			cptr=(unsigned char __code *)address;
...
unsigned char read_sfr (unsigned char x)
{
    ...
    ptr=(__xdata unsigned char *) SFR_CODE_LOC;
...
void set_timer1_priority (void) __naked
{
	__asm__(
...
void JumpToMonitor (void) __naked
{
    __asm__(
	".area JUMPCSEG    (ABS)"
